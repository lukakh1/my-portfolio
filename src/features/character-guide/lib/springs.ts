 ...
 * damping ratio `zeta`. Every animated channel on the robot goes through one
 * of these — refs are never written from closed-form curves directly, which
 * is what made the old robot feel mechanical. Springs give overlap, drag,
 * settle and follow-through "for free".
 *
 * - zeta >= 1: exact critically-damped exponential step (unconditionally
 *   stable at any dt).
 * - zeta < 1: semi-implicit Euler; very bouncy springs (zeta < 0.4, e.g. the
 *   antenna) are sub-stepped at 120 Hz so a frame spike can't blow them up.
 */
export class Spring1 {
  ...
  /** Impulse — the main way to add "life" (footfalls, landings, boings). */
  kick(v: number) {
    this.vel += v;
  }
...
/** Three Spring1s sharing (f, zeta) — for Euler triples / 2D+1 channels. */
